...
// console.log(category);
  return (
    ...
    <button className="bg-blue-600 px-6 mx-2 rounded-md shadow-md text-white">search</button>
        ...
        {/* filter  */}
        <button className="bg-blue-600 px-6 mx-2 rounded-md shadow-md text-white mt-5 sticky top-36 z-40 md:hidden" onClick={handleOpen}>Filter</button>
     <div className={`sticky md:top-40 top-56 z-50  md:translate-x-0 ${open?'block':'-translate-x-96'} transition-all`}>
      ...
      <div>
        ...
        <div className="flex flex-col  mx-9 p-2">
          {categoryType.map((item,i)=>(
            <button className={`filterBtn ${category===item.value? 'bg-black-500 text-white': 'block'}`} onClick={()=>handelCategory(item.value)}>{item.type}</button>
          ))}
        </div>
        <p className="font-bold text-2xl text-black-500">Type</p>
        <div className="flex flex-col  mx-9 p-2">
          {guideType.map((item,i)=>(
            <button className={`filterBtn ${type===item.value? 'bg-black-500 text-white': 'block'}`} onClick={()=>handelType(item.value)}>{item.type}</button>
          ))}
        </div>
      </div>
     </div>
